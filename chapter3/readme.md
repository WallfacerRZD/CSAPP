...
### 指针
指针类型不是机器代码的一部分, 它是C语言提供的一种抽象, 帮助程序员避免寻址错误

一个数组名字可以像指针变量一样引用, 但不能修改.

将指针强制转换成另一种类型,只改变它的类型不改变它的值

### 内存越界和缓冲区溢出

C对数组应用不进行任何边界检查,而且局部变量和状态信息都存放在栈中

对越界的内存写值可进行缓冲区溢出攻击(修改栈中的返回地址)

### 对抗缓冲区溢出攻击

1. **栈随机化**: 在程序开始时, 在栈上分配一段0-n字节之间的随机大小空间.使得程序每次执行时栈位置都不同

2. **栈破坏检测**: 在栈帧中任何局部缓冲区的栈状态之间存储一个特殊的金丝雀(哨兵)值.在函数返回时检测该值是否被修改,如果是程序异常终止.

3. **限制可执行代码区域**: 限制内存区域只能进行"读" "写" "执行"

### 变长栈帧

变长数组意味着无法在编译时确定栈帧的大小, 因此需要变长栈帧

x86-64代码使用%rbp作为帧指针(基指针),在函数整个执行过程中,都使得%rbp指向那个时刻栈的位置然后用固定长度的局部变量相对于%rbp的偏移量来引用他们

函数返回时使用leave指令恢复父过程的栈帧
