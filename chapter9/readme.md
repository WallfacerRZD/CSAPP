...
1. Linux文件系统中的普通文件: 例如一个可执行目标文件, 文件区的每一片包含一个虚拟页面的初始内容. 按需进行页面调度.   
2. 匿名文件: 匿名文件由内核创建, 包含的全是二进制零.

#### 再看共享对象
一个对象可以被映射到虚拟内存的一个区域, 要么作为共享对象要么作为私有对象.   

一个共享对象的任何写操作, 对那些把这个共享对象映射到它的虚拟内存的那些进程也是可见的.  

私有对象使用写时复制技术被映射到虚拟内存中. 两个进程将一个私有对象映射到它们虚拟内存的不同区域, 共享这个对象的同一个物理副本. 只有在进程试图写私有对象时才在物理内存中创建新副本.  

#### 再看fork函数
fork创建的新进程与父进程的虚拟内存相同. 两个进程中的每个页面都标记为只读, 每个区域结构都标记为私有的写时复制.   
...
#include<unistd.h>
#include<sys/mman.h>
void *mmap(void *start, size_t length, int prot, int flags, int fd, off_t offsest);
```  

mmap函数要求内核创建一个新的虚拟内存区域, 最好是从地址start开始的一个区域, 并将文件描述符fd指定的对象的一个连续的片映射到这个新的区域.  

## 动态内存分配
动态内存分配器维护进程虚拟内存中的堆区, 对每个进程内核维护一个brk变量指向堆的顶部.堆向高地址增长.  

分配器有两种基本风格:
- 显示分配器: 要求应用显示地释放任何已分配的块
- 隐式分配器: 分配器检测一个已分配块何时不再被程序所使用, 那么就释放这个块
